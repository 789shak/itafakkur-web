  ...
  return (
    <div className="mx-auto max-w-md px-6 py-16">
      ...
      <button
        onClick={withGoogle}
        disabled={loading}
        className="w-full h-11 rounded-full bg-white border border-border text-[#3A2812] text-sm font-medium hover:bg-white/90 transition-colors disabled:opacity-50 flex items-center justify-center gap-3 mb-3"
      >
      ...
      <form onSubmit={mode === 'password' ? withPassword : withMagicLink} className="space-y-3">
        <input
          ...
          required
          className="w-full h-11 px-4 rounded-full bg-surface border border-border text-sm text-brown placeholder:text-muted focus:outline-none focus:border-gold"
        />
        {mode === 'password' && (
          <input
            type="password"
